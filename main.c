...
void merge(int* arr, int asc, int l, int m, int r) {
  ...
  for(int i = 0; i < len_l; i++) arr_l[i] = arr[i+l];
  for(int i = 0; i < len_r; i++) arr_r[i] = arr[i+m+1];
  int i,j,k;
...
void counting_sort(int* arr, int n, int asc) {
    ...
    int index[max+1];
    int result[n];
    for(int i = 0; i < max+1; i++) index[i] = 0;
    for(int i = 0; i < n; i++) index[arr[i]]++;
    if(asc) {
      for(int i = 1; i < max+1; i++) index[i] += index[i-1];
    } else {
      for(int i = max-1; i >= 0; i--) index[i] += index[i+1];
    }
    for(int i = 0; i < n; i++) result[--index[arr[i]]] = arr[i];
...
int main () {
    arr input = input_array();
    printf("\nYour array: ");
    print_arr(input.array, input.size);
    int swap;
    int alg;
    int asc;
    ...
    printf("1- Yes.\n");
    scanf("%i", &swap);
    if(swap) even_odd_swap(input.array, input.size);
    ...
    scanf("%i", &asc);
    if(alg == 1) {
      quicksort(input.array, input.size, asc);
    } else if(alg == 2) {
      shellSort(input.array, input.size, asc);
    } else if(alg == 3) {
      merge_sort(input.array, input.size, asc);
    } else if(alg == 4) {
      counting_sort(input.array, input.size, asc);
    } else if(alg == 5) {
      odd_sort(input.array, input.size, asc);
    } else {
      exit(0);
    }
    printf("Sorted array: ");
